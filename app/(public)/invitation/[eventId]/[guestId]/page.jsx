    ...
    const fetchMapCenter = async () => {
      if (
        ...
        try {
          ...
          if (eventWithMap && eventWithMap.mapCoordinate) {
            setMapCoordinate(eventWithMap.mapCoordinate);
          } else {
            setMapCoordinate(DEFAULT_MAP_CENTER); // fallback: SF
          }
        } catch {
          setMapCoordinate(DEFAULT_MAP_CENTER);
        } finally {
          setMapLoading(false);
        }
      } else if (event && event.mapCoordinate) {
        setMapCoordinate(event.mapCoordinate);
      }
    };
    fetchMapCenter();
  }, [event]);
