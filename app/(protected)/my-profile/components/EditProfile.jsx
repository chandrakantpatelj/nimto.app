...
// Example timezone list (replace with a full list if needed)
const TIMEZONES = [
    'UTC',
    'America/New_York',
    'America/Chicago',
...
export function EditProfile({ user }) {
    ...
    // Keep search input in sync with selected timezone
    useEffect(() => {
    ...
    const handleSaveProfile = async () => {
        ...
        try {
            ...
            const res = await fetch('/api/user-management/users/[id]/update-userprofile', {
                method: 'POST',
                        ...
                        <div>
                            ...
                            <div className="relative">
                                <Input
                                    ...
                                    className={`${isDark ? 'bg-gray-800 border-gray-700 text-white' : 'bg-white border-gray-300'} mb-2`}
                                    autoComplete="off"
                                />
                                <div
                                    className={`absolute z-10 w-full bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded shadow max-h-48 overflow-y-auto ${showTzDropdown && filteredTimezones.length > 0 ? '' : 'hidden'}`}
                                    tabIndex={-1}
                                ...
                                >
                                    {filteredTimezones.map((tz) => (
                                        <div
                                            key={tz}
                                            className={`px-3 py-2 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 ${tz === timezone ? 'bg-gray-100 dark:bg-gray-700 font-semibold' : ''}`}
                                            onMouseDown={() => {
                                                setTimezone(tz);
                                    ...
                                    {filteredTimezones.length === 0 && (
                                        <div className="px-3 py-2 text-gray-500">No timezones found</div>
                                    )}
                                </div>
                            </div>
                            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
