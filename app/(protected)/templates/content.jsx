...
export function EnhancedTemplateManagement() {
  ...
  const handleOrientationChange = (value) => {
    setFilters({
      ...filters,
      orientation: value === 'all' ? null : value,
    });
  };
  ...
  const hasActiveFilters = filters.orientation || filters.premium || 
    filters.trending || filters.featured || filters.new || selectedCategory || searchQuery;
      ...
      {categories.length > 0 && (
        <div className="bg-white rounded-2xl p-6 sm:p-8 shadow-sm border border-gray-100 mx-4 sm:mx-0">
          ...
          <div className="flex flex-wrap justify-center gap-8 sm:gap-10 md:gap-12">
            ...
            >
              <div className={`w-24 h-24 sm:w-28 sm:h-28 md:w-32 md:h-32 rounded-full shadow-lg transition-all duration-300 group-hover:scale-105 group-hover:shadow-xl flex items-center justify-center overflow-hidden ${
              ...
              <span className={`mt-4 text-sm sm:text-base font-semibold uppercase tracking-wide transition-colors duration-300 ${
                selectedCategory === null ? 'text-purple-600' : 'text-gray-700'
              }`}>
                All Templates
              </span>
            </div>
                ...
                >
                  <div className={`w-24 h-24 sm:w-28 sm:h-28 md:w-32 md:h-32 rounded-full shadow-lg transition-all duration-300 group-hover:scale-105 group-hover:shadow-xl flex items-center justify-center overflow-hidden ${
                    ...
                    {category.thumbnailUrl ? (
                      ...
                      <span className={`font-bold text-2xl sm:text-3xl md:text-4xl ${
                        selectedCategory === category.slug ? 'text-white' : 'text-gray-600'
                      }`}>
                  ...
                  <span className={`mt-4 text-sm sm:text-base font-semibold uppercase tracking-wide transition-colors duration-300 ${
                    selectedCategory === category.slug ? 'text-purple-600' : 'text-gray-700'
                  }`}>
                    {category.name}
                  </span>
                </div>
        ...
        <div className="border-t border-gray-100 pt-4 sm:pt-6">
          <div className="flex flex-col sm:flex-row sm:flex-wrap items-start sm:items-center gap-4 sm:gap-6">
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <span className="font-semibold text-gray-800 text-base sm:text-lg">Filters:</span>
            </div>

            {/* Filter Controls */}
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 w-full sm:w-auto">
              {/* Orientation Filter */}
              <Select value={filters.orientation || 'all'} onValueChange={handleOrientationChange}>
              ...
              {/* Premium Filter */}
              <Select value={filters.premium || 'all'} onValueChange={handlePremiumChange}>
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-4 md:gap-6 w-full sm:w-auto">
              <div className="flex items-center space-x-2 sm:space-x-3">
                ...
                />
                <label htmlFor="trending" className="text-xs sm:text-sm font-semibold text-gray-700 cursor-pointer">
                  Trending
                </label>
              </div>
              ...
              <div className="flex items-center space-x-2 sm:space-x-3">
                <Checkbox 
                  ...
                  className="w-4 h-4 sm:w-5 sm:h-5"
                />
                <label htmlFor="featured" className="text-xs sm:text-sm font-semibold text-gray-700 cursor-pointer">
                  Featured
                </label>
              </div>
              ...
              <div className="flex items-center space-x-2 sm:space-x-3">
                <Checkbox 
                  ...
                  className="w-4 h-4 sm:w-5 sm:h-5"
                />
                <label htmlFor="new" className="text-xs sm:text-sm font-semibold text-gray-700 cursor-pointer">
                  New
                </label>
              </div>
            </div>
          ...
          {hasActiveFilters && (
            <div className="mt-4 sm:mt-6 pt-4 sm:pt-6 border-t border-gray-100">
              ...
              <div className="flex flex-wrap gap-2 sm:gap-3">
                {selectedCategory && (
                  <Badge variant="secondary" className="px-2 sm:px-3 py-1 bg-purple-100 text-purple-700 border border-purple-200 text-xs sm:text-sm">
                ...
                {filters.orientation && (
                  <Badge variant="secondary" className="px-2 sm:px-3 py-1 bg-blue-100 text-blue-700 border border-blue-200 text-xs sm:text-sm">
                    {filters.orientation}
                    <X 
                ...
                {filters.premium && (
                  <Badge variant="secondary" className="px-2 sm:px-3 py-1 bg-green-100 text-green-700 border border-green-200 text-xs sm:text-sm">
                    {filters.premium}
                    <X 
                ...
                {filters.trending && (
                  <Badge variant="secondary" className="px-2 sm:px-3 py-1 bg-orange-100 text-orange-700 border border-orange-200 text-xs sm:text-sm">
                    Trending
                    <X 
                ...
                {filters.featured && (
                  <Badge variant="secondary" className="px-2 sm:px-3 py-1 bg-yellow-100 text-yellow-700 border border-yellow-200 text-xs sm:text-sm">
                    Featured
                    <X 
                ...
                {filters.new && (
                  <Badge variant="secondary" className="px-2 sm:px-3 py-1 bg-pink-100 text-pink-700 border border-pink-200 text-xs sm:text-sm">
                    New
                    <X 
