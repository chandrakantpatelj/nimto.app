...
const SeparateDateTimeFields = ({
    ...
    const [startTimeValue, setStartTimeValue] = useState(
        startValue ? format(new Date(startValue), 'HH:mm') : '12:00'
    );
    ...
    const [endTimeValue, setEndTimeValue] = useState(
        endValue ? format(new Date(endValue), 'HH:mm') : '12:00'
    );
    ...
    const DateTimePickerContent = ({
        ...
        useEffect(() => {
            if (timeListRef.current && timeValue) {
                const btn = timeListRef.current.querySelector(`[data-time="${timeValue}"]`);
                if (btn) {
                    timeListRef.current.scrollTop =
                        btn.offsetTop - timeListRef.current.clientHeight / 2 + btn.clientHeight / 2;
                }
            }
        }, [timeValue]);
                ...
                <div className="w-64 sm:w-72 p-4 sm:p-4">
                    <div className="space-y-4 sm:space-y-4">
                        ...
                        <div>
                            ...
                            <div
                                className="h-48 sm:h-48 overflow-y-auto space-y-1 pr-2 sm:pr-2"
                                ref={timeListRef}
                            >
                                {timeSlots.map((time) => (
                                    <button
