...
export const convertTimezone = (dateString, fromTimezone, toTimezone) => {
    try {
        ...
        // Step 1: Interpret dateString as being in fromTimezone
        const dateInFromTz = new Date(
            new Date(dateString).toLocaleString('en-US', { timeZone: fromTimezone })
        );
        // Step 2: Format that date in toTimezone
        const toTime = dateInFromTz.toLocaleString('en-US', { timeZone: toTimezone });
...
export const isUserInEventTimezone = (eventTimezone) => {
    try {
        ...
        console.warn('Error checking user timezone:', error);
        return false;
    }
};
