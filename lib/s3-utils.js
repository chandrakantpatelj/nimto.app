...
// Generate direct S3 URL (no proxy, no encoding)
export function generateDirectS3Url(imagePath) {
  if (!imagePath) return null;
...
// Generate proxy URL for S3 object (to avoid CORS issues) - DEPRECATED
export function generateProxyUrl(imagePath) {
  if (!imagePath) return null;

  const { bucket, region, endpoint } = getS3Config();
  if (!bucket) return null;

  // Generate S3 URL without double encoding
  let s3Url;
  if (endpoint) {
    // For S3-compatible services (DigitalOcean Spaces, etc.)
    s3Url = `${endpoint}/${bucket}/${imagePath}`;
  } else {
    // For AWS S3 - follow AWS standard path structure (bucket/bucket/path)
    s3Url = `https://${bucket}.s3.${region}.amazonaws.com/${bucket}/${imagePath}`;
  }

  // Use the standard image-proxy endpoint
  return `/api/image-proxy?url=${s3Url}`;
}
